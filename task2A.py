...
# Diffie Hellman Key Exchange
    ...
    alice = Person('Alice', p, g, "Hi Bob!")
    bob = Person('Bob', p, g, "Hi Alice!")
    # add mallory to the key exchange, sets g to 0 because she won't need it
    mallory = Person('Mallory', p, 0, "ψ(｀∇´)ψ")
    # mallory sets her public key to p
    mallory.pub_key_mine = p
    ...
    # Mallory sends her public key which is just p, without bob or alice knowing
    send_pubkey(mallory, alice)
    send_pubkey(mallory, bob)


    generate_secret(alice)
    generate_secret(bob)
    # mallory generates her secret without ever having received/generated a public key
    ...
    """ mallory deciphers alice's message """
    ...
    """ mallory deciphers bob's message """
    decipher = AES.new(mallory.pkey, AES.MODE_CBC, iv)
    message = decipher.decrypt(ciphertext)
    print(str(unpad(message, 16)))
